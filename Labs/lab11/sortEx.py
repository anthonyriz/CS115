...
#L = [0,2,4,6,3,0,5] #print [0,2,3,4,6,0,5]
#print(insertV1(L, 4))
...
def search(L, i, x):
    ...
    for j in range(1, i+1):
        ...
        if L[i-j] <= x:
            return (i-j) + 1
    return 0 #prevents 'None' returning
            

#L = [0,2,4,6,3,0,5]
#print(search(L, 3, 3)) # print 2

#L = [1,2,3,4,1]
#print(search(L, 3, 1)) #print 1

#L = [1,3,5,5]
#print(search(L, 3, 6)) #print 3

#L = [5]
#print(search(L, 1, 2)) # print 0

def testSearch():
    '''test function for search(L, i, x)'''
    # in middle
...
def insertV2(L, i):
    ...
    inital = L[i]
    for j in range (i - search (L, i, inital)):
        swap(L, i-j, i-j-1)
...
def testV1():
    '''test function for insertSortV1(L, i)'''
    testSort(insertSortV1)

def testV2():
    '''test function for insertSortV2(L, i)'''
    testSort(insertSortV2)
    
def testSort(sortFun):
    '''test function for sort functions'''
    
    def test(L):
        '''provides lists to be sorted'''
        print(L)
        sortFun(L)
        print("sorted?", L)
        assert isSorted(L)
...
def letterCounts(fname):
    ...
    letterCount = {"non-letter": 0}
    file = open(fname, "r")
   

    for line in file:
        for Letter in line:
            if letter(Letter) not in letterCount:
                letterCount[Letter] = 1
            elif letter(Letter) == "non-letter":
                letterCount["non-letter"] += 1
            else: 
                letterCount[Letter]+=1
    ...
    for Letter,count in letterCount.items():
        if count > 0:
            letterCountList.append((count, Letter))       
        
    insertSortV2(letterCountList)
    letterCountList.reverse()

    return letterCountList

#print(letterCounts("Labs/lab11/small_file.txt")) 
